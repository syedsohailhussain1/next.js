...
Do not treat the copied description as an agent-written one: it is the contributor's own text, so it is copied rather than rewritten, and it carries no `<!-- NEXT_JS_LLM -->` marker.
