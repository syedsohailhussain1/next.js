...
describe('strict-route-matching-known-limitations', () => {
  ...
  it('renders a named-only loader tree with metadata and CSS', async () => {
  ...
  describe('metadata error delivery', () => {
    it('documents that metadata errors are swallowed on initial load', async () => {
    ...
    it('documents that a metadata redirect is swallowed only on initial load', async () => {
      ...
      // redirect just like the soft navigation does.
      expect(response.status).toBe(200)
      expect(response.headers.get('location')).toBeNull()
