...
impl TurboTasksBackend {
    ...
    pub fn new(mut options: BackendOptions, backing_storage: TurboBackingStorage) -> Self {
        ...
        // persists/GCs. Refuse that combination with a warning rather than leaking. `options.gc`,
        // when set, overrides the env (tests use it to run GC deterministically without racing the
        // process-global env).
