...
enum TaskAccess {
    ...
    ///
    /// The existence check is enabled in **all builds**: a nonexistent task panics. This also
    /// converts a corrupt or truncated on-disk cache into a panic, which is intended — there is no
    /// recovery path for reading a cell on a task missing from disk, and discarding the cache
